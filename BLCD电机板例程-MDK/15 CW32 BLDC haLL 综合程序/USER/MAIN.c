							 ...
							 Menu=0;
							       ...
							       if(Menu==0)
											...
											else if(Menu==1)
											{
												...
												sprintf(temp_buff,"I(C):%d           ",CanshuI);	//显示电流温度
											}
											else if(Menu==2)
											{
											yhj=0;
												sprintf(temp_buff1,"buzz :%d      ",yhj);  //蜂鸣器参数
											sprintf(temp_buff,"temp:%0.1fC      ",canshu/10);	//显示电流温度
											}
			...
			if(TimeCountkey>=500) //500ms刷新显示
			{
				TimeCountkey=0;
											...
											else if(Menu==2)
											{
												...
												sprintf(temp_buff1,"buzz :%d       ",yhj);  //输出显示占空比
												sprintf(temp_buff,"temp:%0.1fC      ",canshu/10);	//显示电流温度
											}
				I2C_OLED_ShowString(0,0,temp_buff1);
				I2C_OLED_ShowString(0,15,temp_buff);	
